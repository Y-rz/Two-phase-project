      ...
      <div class="box-body table-responsive no-padding">
        ...
        @foreach($users as $k=>$v)
          <tr>
            ...
            <a href="" class="userBan" sid="{{$v->uid}}" once="{{$v->status}}">
                <span class="label bg-black">禁言</span>
			      </a>
...
<!-- ajax用户删除 -->
  ...
  $('.userBan').click(function(){
   ...
   if(status == '0'){
 	status = '1';
    //发送ajax
    $.post('/admin/user/ban',{id:id,status:status},function(data){
        if(data == 1){
          //获取提醒信息
          links.attr('once','1');
           links.parents('tr').children('td:eq(11)').html('<span class="label bg-black">禁言</span>');
          $('#successMessage').text('禁言成功').show(1000);
          setTimeout(function(){
            $('#successMessage').hide(1000);
          },2000);
        }
	...
	$.post('/admin/user/ban',{id:id,status:status},function(data){
          if(data == 1){
            //获取提醒信息
            links.attr('once','0');
           links.parents('tr').children('td:eq(11)').html('<span class="label label-success">正常</span>');
            $('#successMessage').text('取消禁言').show(1000);
            setTimeout(function(){
              $('#successMessage').hide(1000);
            },2000);
          }
